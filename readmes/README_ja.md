...
<div align="center">
  ...
  <p>
    openteams は、個人開発者が自分で管理できる AI チームとともに、ソフトウェアをより速く計画、構築、リリースするための、オープンソースかつローカルファーストの AI デスクトップアプリです。Claude Code、Codex、Gemini CLI、組み込みの openteams-cli など、16 種類のコーディングエージェントに対応しています。
  </p>
...
| Pi | インストール不要（`npx` 経由で実行） |
| Hermes | `pip install hermes-agent` |
| Kiro CLI | `curl -fsSL https://cli.kiro.dev/install \| bash` |
| DeepSeek Harness（ACP プレビュー） | アプリ内のバージョン固定セットアップコマンドで[公式ソース](https://github.com/deepseek-ai/deepseek-harness)をビルドしてください。OpenTeams は ACP サーバーを Node で直接起動します（`npx` は使用しません） |
| OpenTeams CLI | openteams に同梱 |

📚 [その他のエージェントインストールガイド](https://doc.openteams-lab.com/getting-started)

## 主な更新
- **2026.08.04 (v1.0.9)**
  - Pi agent 対応（承認、ランチャー、MCP 拡張を含む ACP ベースの新ランタイム）
  - ワークフローランタイムの強化：プロンプト安全性、予算上限、レビュアーループ不変条件、プロンプトごとの予算割り当て
  - チャット agent アクティビティログのリデザインで可読性を向上；共有 markdown レンダラーでアクティビティログをレンダリング
  - Qoder CLI 対応；ワークフロー詳細およびログ UI を改善
  - agent インストール通知と OAuth ステータス確認
- **2026.07.27 (v1.0.5)**
  - ACP 承認と agent 権限設定に対応
  - ワークフローノードスキップ機能を追加
  - Universe app エグゼキューター v1.0
  - 権限 UI と受信トレイ通知スタイルを改善
  - Kimi を ACP プロトコルに変換；`~/.openteams/opencode` の数とサイズを管理
- **2026.07.20 (v1.0.0)**
  - 最初の安定版：カスタム AI メンバー、agent 自動モード、マージコンフリクトウィンドウ
  - macOS DMG インストーラーのスタイリング；新規ユーザー向けコマンドヘルプダイアログ
  - agent メッセージ内のリンクアクセスと Enter でのダイアログ確認に対応
- **2026.05.20 (v0.4.4)**
  - Workflow モード beta 版
...
- [ ] **より多くのエージェント統合** — Kilo Code、openclaw など、よく使われるエージェントをさらに統合します。
...
### Windows のトラブルシューティング（任意）

Windows で `pnpm run dev` の実行時に `error: no such command: watch` と表示された場合は、`cargo-watch` をインストールしてから開発コマンドを再実行してください。このエラーが発生した場合にのみインストールが必要です。
