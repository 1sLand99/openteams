...
## 주요 업데이트
- **2026.08.04 (v1.0.9)**
  - Pi agent 지원 (승인, 런처, MCP 확장을 포함한 ACP 기반 신규 런타임)
  - 워크플로 런타임 강화: 프롬프트 안전성, 예산 상한, 리뷰어 루프 불변 조건, 프롬프트별 예산 할당
  - 가독성 향상을 위한 채팅 agent 활동 로그 리디자인; 공유 markdown 렌더러로 활동 로그 렌더링
  - Qoder CLI 지원; 워크플로 상세 및 로그 UI 개선
  - agent 설치 알림 및 OAuth 상태 확인
- **2026.07.27 (v1.0.5)**
  - ACP 승인 및 agent 권한 설정 지원
  - 워크플로 노드 건너뛰기 기능 추가
  - Universe app 실행기 v1.0
  - 권한 UI 및 받은편지함 알림 스타일 개선
  - Kimi를 ACP 프로토콜로 변환; `~/.openteams/opencode` 수 및 크기 관리
- **2026.07.20 (v1.0.0)**
  - 첫 안정 버전: 커스텀 AI 멤버, agent 자동 모드, 병합 충돌 창
  - macOS DMG 설치 프로그램 스타일링; 신규 사용자용 명령 도움말 대화상자
  - agent 메시지 내 링크 방문 및 Enter 키 대화상자 확인 지원
- **2026.05.20 (v0.4.4)**
  - Workflow 모드 beta 버전
...
Windows에서 `pnpm run dev` 실행 시 `error: no such command: watch` 오류가 표시되면 `cargo-watch`를 설치한 후 개발 명령을 다시 실행하세요. 이 오류가 발생한 경우에만 설치하면 됩니다.
