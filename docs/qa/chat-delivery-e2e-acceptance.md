...
| CDD-009 | A 运行失败且 B 已排队；观察 blocked；执行 continue；释放 B；另模拟 pre-bind/dispatch 失败。 | A 记录失败原因并阻塞 B；continue 只跳过/解除失败项并启动 B；B 完成后队列恢复可用，不能重复 claim。pre-bind 或 dispatch CAS 后变为 `Failed`/`Skipped` 的 delivery 必须持久保存非空 `failure_reason`。 | `delivery_failure_blocks_continue_and_starts_next` + runner 尾项门禁 |
