---
id: literature_research_team
name: Equipo de investigación bibliográfica
description: Investigación de literatura con navegador, selección adaptada al área, síntesis de evidencia y revisión independiente.
workflow_steps:
- title: Delimitar y planificar
  description: Definir la pregunta, los límites, las bases y revistas o conferencias prioritarias, y la evidencia necesaria.
- title: Buscar y sintetizar
  description: Buscar literatura pública con el navegador, verificar fuentes clave y sintetizar hallazgos con sus límites.
- title: Revisar y entregar
  description: Comprobar de forma independiente las afirmaciones decisivas, corregir problemas y entregar un resultado trazable.
---

Responder la pregunta de investigación con literatura pública verificada mediante el navegador.
- El Researcher delimita el trabajo, asigna búsquedas y sintetiza la evidencia.
- El Assistant consulta bases autorizadas y las principales revistas o conferencias del área; cada publicación entregada incluye título, enlace, fecha, aporte principal y relación con el tema.
- El Reviewer verifica de forma independiente las fuentes decisivas y devuelve `PASS`, `NEEDS_REPAIR` o `ESCALATE`.
- Declarar vacíos de evidencia y límites de acceso; la aceptación final corresponde al usuario.
